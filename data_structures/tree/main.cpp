...
class Tree{
	...
	//Lab
	private: int getDepthAtNode(Node* node=nullptr){
		if (node==nullptr){return 0;}
		int a = getDepthAtNode(node->left);
		int b = getDepthAtNode(node->right);
		return (a>b?a:b)+1;
	}
	public:

	int getMaxDepth(){
		return getDepthAtNode(root);
	}
...
{
	Tree x;
	cout<<"M Depth:"<<x.getMaxDepth()<<endl;
	x.add(10);
	cout<<"M Depth:"<<x.getMaxDepth()<<endl;
	x.add(5);
	cout<<"M Depth:"<<x.getMaxDepth()<<endl;
